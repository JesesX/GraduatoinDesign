...
subsystemTypes = {'8PSK 8/9'};

% EsNodBs       = 0.1 : 0.1 : 4                                            %信噪比(dB)
EsNodBs = 10.5;
numFrames     = 700;                                                         %帧的个数
BerLDPCs      = zeros(numel(subsystemTypes), numel(EsNodBs))               %初始化误码率

EsNodB_E = zeros(numel(EsNodBs), numel(numFrames))
SNR_E = zeros(numel(EsNodBs), numel(numFrames))
...
for j = 1 : numel(subsystemTypes)
    ...
    for i = 1 : numel(EsNodBs)
        ...
        for frameCnt=1:numFrames
            ...
            BerLDPCs(j, i) = BerLDPCs(j, i) + berLDPC(1);                   %计算LDPC译码误码率
            numIterVec(frameCnt) = numIter;
            noiseVar   = meanCalc(varCalc(chanOut - modOut));
            
            %%%%%  计算LDPC译码后信噪比 %%%%%
            K = numel(ldpcDecOut)                                          %当前编码调制方案下ldpc译码后一帧的码元个数
            %%% 计算M2  %%%
            for k = 1 : K
                M_2 = 
            end
...
save('F:\Corrosponding_Engineering\毕业设计\matlab\提交\DVB_S2_5_15', 'subsystemTypes', 'EsNodBs', 'BerLDPCs')
